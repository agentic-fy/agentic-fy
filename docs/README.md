# Documentação do agentic

Bem-vindo. Este é o ponto de partida de tudo sobre o agentic.

O agentic ajuda você e seu assistente de IA a **concordarem sobre o que construir antes de escrever qualquer código.** Você descreve a mudança, a IA rascunha uma spec curta e uma lista de tarefas, vocês dois olham para o mesmo plano, e então o trabalho acontece. Sem descobrir no meio do caminho que a IA construiu a coisa errada.
...
O agentic tem duas metades: uma ferramenta de linha de comando que você roda no terminal, e um servidor MCP que expõe o workflow como ferramentas para o seu assistente de IA. Saber qual é qual evita o ponto de confusão mais comum.
...
**Só quero pôr pra funcionar.** [Instale](getting-started.md#instalacao), rode `agentic init`, e conecte o servidor MCP ao seu assistente (veja [Comandos](commands.md#mcp)).
...
| [Comandos](commands.md) | Referência de cada comando do CLI `agentic` |
...
```text
1. Instalar       npm install -g @agentic-fy/agentic
2. Inicializar    cd seu-projeto && agentic init
3. Explorar       agentic explore                 ← opcional, mas um ótimo hábito
4. Propor         agentic propose add-dark-mode
5. Aplicar        agentic apply
6. Verificar      agentic verify
7. Arquivar       agentic archive
```

Os comandos rodam no seu terminal. Para deixar o assistente de IA conduzir o fluxo sozinho, conecte o servidor MCP (`agentic mcp`) — veja [Comandos](commands.md#mcp).

## Onde conseguir ajuda

- **npm:** [@agentic-fy/agentic](https://www.npmjs.com/package/@agentic-fy/agentic)
